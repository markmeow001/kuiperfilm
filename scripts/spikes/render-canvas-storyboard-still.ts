...
async function main() {
  ...
  try {
    ...
    const inputProps = { items: sources.map((source, index) => ({ title: `镜头 ${index + 1} · ${path.basename(source, '.jpg')}`, imageUrl: `${baseUrl}/${String(index + 1).padStart(2, '0')}.jpg` })), columns: 4 as const, showShotNumber: true }
    ...
    const sample = async () => { peakBrowserTreeRssBytes = Math.max(peakBrowserTreeRssBytes, await readProcessTreeRssBytes(browserPid)) }
    await sample()
    monitor = setInterval(() => { void sample() }, 50)
    ...
    process.stdout.write(`${JSON.stringify({ output, width: composition.width, height: composition.height, items: 25, real4kInputCount: 25, elapsedMs, outputBytes: file.size, browserPid, peakBrowserTreeRssBytes })}\n`)
  } finally {
