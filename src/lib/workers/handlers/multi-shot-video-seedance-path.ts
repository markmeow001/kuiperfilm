...
export async function runMultiShotSeedanceComposite(params: {
  ...
  //   2. (2026-06-13) dialogue-driven — size the composite from each panel's
  //      spoken line so dialogue always has airtime (aligns Seedance with
  //      fal / atlascloud / b-path, which already do this). Dialogue is the
  //      primary duration signal per product direction.
  //   3. Panel-count × 2s baseline (silent groups only), clamped 4-15s.
  let duration: number
