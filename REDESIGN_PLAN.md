...
| **flova.ai** *(2026-06-03 logged-in walkthrough)* | Creator economy primitive | **「Skill」概念** = `workflow + model chain + prompt template + workflow constraints` 封裝成命名 + 可署名 + 可分享 bundle；6 個官方 Skill + 12+ 社區 creator Skill 雙層 marketplace；元素庫加 creator attribution + 熱門 badge；左 icon-only sidebar 256px | 漸層 CTA（金橙→綠 gradient）、多層 promo modal 轟炸、頂部彩虹 promo banner、chat-input 置中當 hero、快速生成 paywall（強迫所有 Free user 走 Skill）、passwordless OTP-only login |
...
### Phase 2.5 · Skill primitive 引入（2–3 週，**flova 2026-06-03 真實版分析後新增**）

> 2026-06-03 logged-in walkthrough of flova.ai 揭出一個 KuiperAI 沒有的架構 primitive：
> **Skill = `(workflow + model chain + prompt template + workflow constraints)` 的命名 + 可署名 + 可分享 bundle**。
>
> flova 把產品從「又一個 AI video tool」升級成「creator economy platform」就靠這層。
> KuiperAI 已經有所有 runtime（worker、model picker、visualStyles、multi-shot composite、
> R2V 雙視圖、劇集設定），**沒有的只是命名 + 屬性 + surface 三件事**。
>
> 完整 spec 見 [`IMPL_PREP/R-skill-primitive.md`](./IMPL_PREP/R-skill-primitive.md)。

**Owner:** 1 個 senior frontend + 0.5 個 backend

**輸出：**
- Schema：`Skill` + `SkillInstallation` + enums（`SkillAuthorType`, `SkillStatus`）
- 6 個官方 KuiperAI Skill（mirror flova 的「我的 Skill」6 個 default）：
  - `drama-short-seedance-voice` — 劇情短片視頻 (Seedance 角色音色連戲)
  - `script-driven-cinematic` — 劇本驅動型視頻
  - `product-promo-commercial` — 商品宣傳短片
  - `music-mv-omnihuman` — 音樂 MV
  - `previs-action-storyboard` — 動作預演分鏡視頻
  - `reference-recreation` — 視頻拉片複刻
- `/[locale]/skills` Skill library page（兩段：我的 Skill / 精選 Skill placeholder）
- `/[locale]/skills/[slug]` Skill 詳情頁
- 敘事編輯器 + 新建專案流程加 Skill picker chip
- `Project.originSkillId String?` nullable 欄位，既有 project = NULL = 「自由創作」

**Definition of Done：**
- 6 個官方 Skill 能跑完整 e2e（從 picker → 生成 → 結果）
- Free tier user 可用前 3 個 Skill；後 3 個需要 Pro
- 既有專案 100% 不受影響（向後相容）

**這個階段 NOT 做：** 社區 creator 自製 Skill、Skill marketplace 搜尋、creator 分潤 — 全留到 Phase 3.5。
...
### Phase 3.5 · Skill marketplace + creator economy（4 週）

> Phase 2.5 把 Skill primitive 做出來；Phase 3.5 把它變成 marketplace。
>
> 目標：5-10 位簽約短劇導演當 KuiperAI 首批 partner creator，做 12-20 個官方掛名的 Skill。
> mirror flova 的「精選 Skill」section（@阿娴 / @境在丨米叔 / @NET FLY 那些）。
>
> 完整 spec 見 [`IMPL_PREP/R-skill-primitive.md`](./IMPL_PREP/R-skill-primitive.md) §10。

**Owner：** 1 個 frontend + 1 個 backend + 1 個 BD（找 creator）

**輸出：**
- Skill 提交流程（user-created Skill submission form）
- Admin review pipeline（published 前自動 probe：跑 fixture 驗成功率 ≥ 90%）
- Creator dashboard（看自己 Skill 的 install / run count）
- Run attribution metrics（哪個 Skill 跑了多少次、平均完成率）
- Marketplace browse + search + 熱門 sort
- 「精選 Skill」section feature flag default ON
- 簽 5-10 位 partner creator + 上架 12-20 個官方 partner Skill

**Definition of Done：**
- 至少 20 個 Skill 上架可用
- 任一 user 可提交 Skill，admin review queue 流程跑通
- Skill marketplace 月活 ≥ Skill 總體啟用 user 數的 50%

**Phase 4.0 預告：** Stripe Connect 接入 + creator 分潤（5-10% revenue share）。**Phase 3.5 不做分潤**，先驗 marketplace 黏性。
