# 範例圖生成 Runbook — Style Library 縮圖

> **What**: 為 `visual_styles` 表的 29 個風格各生成一張範例縮圖,放上 CDN,寫回 `thumbnail_url`。
>
> **Why**: 首頁 Project Settings 的 STYLE 選單目前只有純文字按鈕（截圖 2026-05-13）— 沒有預覽圖,user 看不出每個風格實際長什麼樣。範例縮圖讓選擇可視化。
>
> **When**: 一次性 setup,或新增風格時跑單張。已寫進 DB 的 `thumbnail_url` 是 nullable,所以缺圖不會 crash,只是 UI 顯示空白卡。
>
> **Owner**: 任何拿到 Kling Image O1 或 Midjourney 訪問權的開發者。

## 操作流程

1. **準備工具**:挑一個影像生成服務
   - Kling Image O1(已綁在 VOD AIGC 體系,直接用 admin 既有的 Tencent VOD key 走 `CreateAigcImageTask` API)
   - 或 Midjourney v6.1(透過 Discord bot 或 unofficial API)

2. **跑下面 29 個 prompt 各 1 張**:每張用 1:1 方形 1024×1024,固定 negative `no text, no watermark, no logo`

3. **上傳到 CDN**:
   - 命名 convention: `style-thumbs/<style_id>.jpg`(全 lowercase,snake_case)
   - 推薦路徑: COS bucket `system/style-thumbs/` 或 R2 `style-thumbs/`
   - 走 Cloudflare 或 Tencent CDN front 給穩定 URL

4. **回寫 DB**:
   ```sql
   UPDATE visual_styles
   SET thumbnail_url = CONCAT('https://your-cdn.com/style-thumbs/', id, '.jpg'),
       updated_at = NOW()
   WHERE thumbnail_url IS NULL;
   ```
   或一張一張 update:
   ```sql
   UPDATE visual_styles SET thumbnail_url = '...' WHERE id = 'cinematic_realism';
   ```

5. **驗證**:打開 admin UI 的風格選單,29 張縮圖都應該顯示。
