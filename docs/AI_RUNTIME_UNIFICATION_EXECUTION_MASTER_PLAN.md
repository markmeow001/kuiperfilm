...
- 🔄 Phase 11: 竞品对标功能补完（基于 docs/competitor-features/ 分析；Top 5 优先）— 11.5 + 11.1 + 11.2 主要子任務 ✅ 完成；11.3 / 11.4 仍 ⏸；11.1.5 / 11.2.5 reviewer follow-up ⏸
  ...
  - ✅ Phase 11.4: 角色三视图(2026-05-01 Session A `9b68107` 完成 — 重用既有 CHARACTER_REFERENCE_TO_SHEET worker + `CharacterAppearance.imageUrls` JSON array,0 schema 改动)
...
- ✅ 12.x.x v2 EpisodeTabBar 整合 — v2 workspace 每個 page 改讀 URL `?episode=<id>` 取代 `episodes[0]`。複用既有 `src/components/ui/EpisodeTabBar.tsx`(Phase 11.1)掛在 V2WorkspaceShell,支援切集 / 新建 / 重命名 / 刪除。`useCurrentEpisode` hook 為單一狀態源,setCurrentEpisode 用 router.replace 保留其他 URL params。Stage 1 commit `c4b908f`(foundations + ScriptPage),Stage 2 commit `<本 commit>`(StoryboardPage + FinalPage;SubjectsPage / VoicePage 不依賴 episode)。Tests:tests/unit/lib/v2-current-episode.test.ts 6 cases 全綠。
...
- ✅ 2026-07-24 Seedance R2V 多參考圖：改為「一位原片人物對應一張角色卡」，角色依卡片順序、場景接續其後，共用模型的 9 張圖片上限；空白必填角色會先保留位置，UI、Prompt mapping、workflow 與供應商契約四層皆拒絕第 10 張。深度影片仍獨立計算，不占圖片額度。
- ✅ 2026-07-24 短句 AI 補全：角色外觀與場景動態各自提供明確按鍵，500 字輸入，繁中／英文系統提示分流；模型實際輸出上限與估價一致，queue／provider 均不做隱藏重試。相同 request key 由資料庫唯一鍵原子決定唯一任務，active 與 terminal 均回傳原任務；已完成任務若結果暫時無法讀取／解析會保留原 task id 繼續取回。瀏覽器重新整理可恢復原 AI／影片任務；若 sessionStorage 無法保存識別碼，會在上傳或扣費前顯式停止。
- ✅ 2026-07-24 最終驗證：重複計費與續接安全定向 3 files、38 tests 全數通過；完整 regression 為 unit 2260、billing integration 22、API integration 340、chain integration 14，DOM 23 files／107 tests，全部通過；TypeScript、指定 ESLint、diff check 與 `npm run build` 成功。AtlasCloud 付費端到端仍維持下方待使用者明確確認的驗收限制。
...
- ✅ Q-002（已修，2026-09-10，commit `da3d6be`）：`scripts/check-api-handler.ts` 不再 shell out 到 ripgrep，改用 Node `fs` 遞迴走訪 `src/app/api`。除了拿掉外部工具依賴，也不再為每支路由 spawn 一個 `rg` 進程（241 支路由 = 241 次 process 啟動），整支 guard 現在 1.7 秒跑完。驗證：乾淨 exit 0（routes=241）、臨時放入未包 `apiHandler` 的探針路由 exit 1 並指名該檔（證明不是假綠燈）、`npm run test:guards` 全綠。
- ✅ Q-006（已修，2026-09-10，commit `da3d6be`）：`package-lock.json` 有 899 筆 `resolved` 釘在 `registry.npmmirror.com`，境外環境（海外 CI／雲端沙盒／國外貢獻者）連不到該鏡像，`npm ci` 會卡在數萬次連線失敗而永遠裝不完 —— 全新 clone 等於無法建置。已改寫為 `registry.npmjs.org`（1532 筆 resolved 現在同源）；tarball 內容相同、integrity hash 不變，`npm ci` 的完整性驗證通過即為證明，實測由「裝不完」變成 50 秒完成。偏好鏡像者應在本機 `.npmrc` / `npm config` 設定，而非釘進 lockfile。
- ✅ G-5 flaky test（已修根因，2026-09-10）：`tests/helpers/auth.ts` 的 mock state 從模組級 `let` 遷到 `globalThis`（`vi.resetModules()` 不再製造第二份 state）、`resetAuthMockState()` 不再 `doUnmock`（消除「解析到真實 api-auth」的空窗）、mock 主體抽為單一 `createAuthMockModule()` 供 runtime 與 hoisted 兩種風格共用。詳見 `docs/ai-runtime/08-open-gaps.md` G-5。⚠️ 原始 flake 需 Docker 起 MySQL/Redis 的完整 regression 才能重現，本輪環境無 Docker，故「flake 已消失」尚未最終確認，該項在 08-open-gaps.md 保持開啟。
...
2 秒节拍按 5 秒计费还得手剪，且 21 镜 × ≥5s 远超 60–90s 目标。修正：
CANVAS_STORYBOARD prompt 钉死 durationSec 为 5–15 整数、要求把相邻微节拍
（睁眼/伸手/对视）合并进同一镜头用镜内节奏描述、镜头数由目标总时长反推
（尊重剧本标注的时长建议）；解析器把时长正规化到 5–15（表格显示秒数 =
实际生成/计费秒数，契约执行而非隐式回退）；手动编辑输入框 min 5。
