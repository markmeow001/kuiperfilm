...
## Live Composite 虛擬角色合成（2026-07-18 程式階段）

- ✅ 支援透明 PNG／WebP／GIF 與透明影片角色素材，沿用 playground-ref 上傳與專案權限邊界。
- ✅ 角色可設定固定畫面位置，或依 MediaPipe 人物遮罩中心自動追蹤；相鄰 AI 遮罩關鍵影格間採平滑位置插值。
- ✅ 支援人物前方／後方遮擋、位置偏移、比例、旋轉、不透明度、出現區間與影片循環。
- ✅ 角色合成會進入 PNG 影格與影片錄製輸出，既有素材庫／Canvas handoff 可直接接收最終合成結果。
- ✅ 專案 timeline 保存角色素材 key 與全部非破壞性參數；讀取時由伺服器重新簽名，不新增資料表。
- ✅ 驗證：完整 unit、API integration、guards、TypeScript、lint（0 errors）與 production build 全數通過。
- ✅ 本機瀏覽器驗收：實拍影片與角色圖片首次上傳、角色控制面板、640×360 Canvas 合成顯示、頁面錯誤 overlay 與 console error 檢查通過；未觸發會寫入雲端儲存的專案保存操作。
- ⏸ 下一切片：場景物件深度遮擋、光色／陰影匹配、骨架或動作驅動；本階段不宣稱已完成生成式角色動作合成。
