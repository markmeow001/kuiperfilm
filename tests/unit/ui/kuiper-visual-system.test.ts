...
describe('Kuiper visual system', () => {
  ...
  it('requires explicit generation controls and prompt review for live-action reconstruction', () => {
    ...
    expect(controls).toContain('人物外觀參考圖')
    expect(controls).toContain('動作優先')
    expect(controls).toContain('人物外觀參考')
    expect(controls).toContain('高一致性')
    expect(controls).toContain('Seedance 沒有 API 級人物硬綁定')
    expect(studio).toContain("strategy === 'motion-first' ? []")
    expect(studio).toContain('referenceImageKeys: activeReferenceImages.map')
